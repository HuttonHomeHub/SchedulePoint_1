...
| **CAL-05** | Turnaround window | **Base week is entirely non-work; work exists ONLY via a positive exception (01–30 Oct).** Engines that can only *subtract* exceptions from a base week fail outright. This is how a TA window should be modelled — with a calendar, not a constraint. Revision 2 widened the window from 05–16 Oct (fixture `revision_note`): the original 144 h could not hold the chain's 156 h forward, and — the sharper lesson — a window-only calendar gives the **backward** pass no representable late date before the window opens, so A10500's mandatory-breaks-logic case needs the full chain's working time *at or before the pin* (9,360 min) inside the window. A10100's Mandatory Start still pins the chain to 05-Oct, so the forward picture is the original intent unchanged. |
