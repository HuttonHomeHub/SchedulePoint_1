...
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `VITE_GANTT_VIEW` off                        | `view-tsld`/`view-gantt` are not visible (`:2389`, `:2403`); the `Plan view` group has no items and **is not rendered**. The remaining `Scheduling mode` group is the only one, and — because it is index 0 — carries no hairline. **NOT byte-identical to today, and this cell claimed it was** (ux gate, 2026-08-30): the surviving item still carries a labelled segment, so `partitionBySegment` returns a **one-entry** array and the segmented branch renders — a roleless `<div>` around one `role="group"`, where today there is one `role="group"` directly. Inert (a roleless div is flattened out of the accessibility tree; a one-child flex container lays out identically) — but inert was an **assumption** until `toolbar-segments.test.tsx`'s one-segment case existed, and no case reached the path, because the `no segmentLabels` case short-circuits on `!labels`. |
