...
| E34 | The D3 shift must be one **calendar** day, and a working-day shift is a plausible wrong implementation.                             | True (T1)                                          | E17's identity is `finishMilestoneDateInstant(cal, D−1) === rollForwardToWorking(cal, abs(D))`, where D−1 is the previous calendar day. For a Monday D, a working-day shift stores Friday, which reads as the end of Friday: the same instant only by coincidence of the Mon–Fri week, and a different one on any calendar whose Saturday works. A case whose D is a Monday discriminates; an interior weekday does not.                                                                                                                                                                                                                                                                               |
...
- **FC-3, undo is exact, including beside a weekend.** Convert, then Undo, restores all five stored
  date fields and the type byte-for-byte. At least one case uses a stored date that is a **Monday**, so
  its D−1 is a Sunday: a working-day shift (E34) fails it and passes every interior-weekday case. The
  case reuses M0-T3's Friday/Monday fixture. Proven by the journey reading the row back through the
  API, and by the M2-T1 unit case that pins Monday − 1 = Sunday.
...
| Stored date is a Monday                                                                                      | D−1 is the **Sunday**, never the Friday (E34). The unit suite and FC-3 pin it.                                                                                                                                                                                                                     |
