...
| S6  | Every REGION reachable by pointer on the minimap is reachable by keyboard — page-granular (arrows) plus the two extremes (Home/End), the scrollbar convention; pixel-exact single-jump stays a pointer affordance, with exact-day access already served by Go to date (wording corrected at the M4 a11y gate — the earlier phrasing overclaimed) | journey (M3-T7) + axe with `wcag22aa` and `target-size` enabled                                  |
