...
| E11 | The backward side **cannot**, for an activity with duration.                                                       | True                               | That branch calls `nextCalendarDay` (`constraints.ts:276-280`), which calls `parseCalendarDate`. That splits on `-` (`calendar-date.ts:27-30`), so `2026-01-10T08:00` yields `Number('10T08:00')`, which is `NaN`. **M0-T3 corrects this: it throws a `RangeError` from `nextCalendarDay`'s `toISOString`, so today the recalculation fails loudly rather than returning `NaN`. The conclusion (the branch cannot take an instant) stands.** The finish-milestone branch (`:272-273`) and the zero-duration branch (`:274-275`) accept an instant. |
...
> **Corrected by M0 (`m0/red-run.md`).** "Every one" is false as a universal claim. Of the 1,728
> matrix cells, **442 are pessimistic**: 153 forward cells start late and 289 backward cells finish
> tight. That includes 41 of 144 forward and 33 of 144 backward task-to-task cells, and FC-10's own
> 6-day cell. The causes are a lead walked in calendar days onto a weekend, an FF duration subtracted
> in calendar days across a weekend, and E8's rounding on an eight-hour calendar. The defect is that
> the two worlds **disagree**, in either direction. The fix is the same.
...
- **FC-6, cost.** (M0: read "10 upstream plans" as **10 remote plans**. The backward direction needs
  remote plans on both sides, and ADR-0045 §3 keeps the plan graph acyclic, so the fixture uses 5
  upstream and 5 downstream plans. The M0 baseline is in `m0/cost.md`: at 100 links each way, p95
  22.0–29.0 ms with a 7.0 ms spread, and query and calendar counts already equal at 10 and 100.)
  On a fixture of 100 cross-plan edges from 10 upstream plans on 10 distinct
...
> **Corrected by M0.** Two things.
>
> - "Rare" describes this base case only. Off the base case, late and tight cells are common (see the
>   correction under §1).
> - The table assumes a **task** at the remote end and never says so. With a finish-milestone remote,
>   the SS and SF rows shift by one day forward, and the FS and SS rows shift by one day backward.
>
> M0-T2 cannot settle the population either way: the seed catalogue holds no cross-plan links and
> structurally cannot (a `SeedSpec` is one plan). The deployed population arrives with M3's boot log.
