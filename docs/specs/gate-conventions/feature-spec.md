...
| Does the epic add a new `check:*` script?                                  | **This answered "No" and was WRONG as built** — M4 added `check:advisory-agreement`, so CI's hand-written step list DID need a new entry and the epic DID add a CI step. Corrected 2026-09-02, found by the devops gate pass checking that roster against `package.json` rather than believing this cell. The gate had shipped enforced only by a locally-run `pnpm prepush`, for a mechanism whose whole subject is that local-only enforcement is the thing that does not happen. |
