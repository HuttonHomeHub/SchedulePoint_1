...
- **Mapping contract (the load-bearing table):** P6/MSPDI entity → SchedulePoint entity, with the
  ...
  | `CALENDAR.clndr_type` (XER only)                                                             | `Calendar.scope` + `Calendar.projectId` (ADR-0053 §5) | **import:** `CA_Project` → PROJECT (pinned to the target project); `CA_Base` → PROJECT + "promote it" finding, or ORG on the explicit `globalCalendarScope: 'ORG'`; a calendar an imported **resource** holds → **forced ORG** + finding (a resource is org-global). Absent column → PROJECT (silent, it is the default); unrecognised value → PROJECT + reported. **export:** `clndr_type` emitted from `scope` + whether a resource holds it (`CA_Rsrc` > `CA_Base`/`CA_Project`), so an XER round trip preserves the tier. **MSPDI has no equivalent** (`IsBaseCalendar` is calendar _inheritance_, not a tier): import → always PROJECT + reported; export → the tier is **dropped + reported**. |
  | an imported calendar name the target tier already holds                                      | a NEW calendar, `"<name> (imported YYYY-MM-DD)"`      | **never reused** — two calendars can share a name and have different working weeks, so reuse would silently reschedule the import; the rename is a reported `repair`. (Consequently calendars have **no** archived-match/unarchive rule, unlike resources' CQ-4: with no match path there is nothing to unarchive.)                                                                                                                                                                                                                                                                                                                                                                                  |
