...
> **A note on how the old application is cited** (added 2026-09-14, M1). It lives at
> `HuttonHomeHub/SchedulePoint`, read at commit **`cf52029a`**, and every reference to it below
> names a **CSS selector or a function** rather than a line number. That is not a style
> preference: a `basename:lines` citation into another repository is the one form ADR-0076's
> `check:claims` cannot place — it is not a dependency (no package, no store entry, no version
> to pin) and not this repository (absent from `git ls-files`), so the gate correctly refuses
> it and has no register entry to accept. Re-anchoring on the selector is also the better
> citation: `#minimap-viewport` survives an edit above it, and a line number in a tree nobody
> pins is exactly the rot ADR-0076 exists to stop. The commit is the pin.
...
- **The viewport rectangle has no fill.** `TsldMinimap.tsx:435-444` is `border: 1px` +
  ...
  amber band over a 10% amber wash** (the `#minimap-viewport` rule).
...
**The old app.** Its `#minimap-viewport` rule — `border: 2px solid var(--secondary-color)`
over `background-color: rgba(252, 163, 17, 0.1)`, `top: 0; height: 100%`.
`--secondary-color: #fca311` (declared in `static/css/main.css`), and `rgba(252,163,17,0.1)` is that exact
...
border, 4px radius, `0 2px 4px rgba(0,0,0,0.2)` (its `#minimap-container` rule). Ours is
...
| **Per-activity colour scatter**      | **Decline — it collides with our semantics**    | Colour in SchedulePoint _means_ criticality (`palette.ts:162-164`; ADR-0097 "cool means interface, warm means attention"; ADR-0102 set the ladder). The old field was user-chosen (`activity-editor.js`, `populateEditor`'s `colorSelect.value`) from **eight** options **including `red` and `orange`** (its `colorOptions` array) — the exact two hues that mean critical and near-critical here. Reproducing it would make a user-chosen "red" activity indistinguishable from a critical one |
...
- The old application: `/home/user/schedulepoint` @ `cf52029a` —
  `templates/schedule-canvas.html` (`drawMinimap`), `static/css/schedule-canvas.css`
  (`#minimap-container`, `#minimap-viewport`, `.minimap-toggle`), `static/js/activity-editor.js`
  (`colorOptions`)
