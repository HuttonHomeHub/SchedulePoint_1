...
| Caller lacks `cost:read`                    | Cost fields come back `null` and are indistinguishable from unset — so the Cost tab is **not rendered at all**, the one deliberate exception to shade-never-hide (ADR-0060 §6). Shading would show blank disabled money fields, which claims the activity has no cost rather than that the reader may not see it. An earlier draft of this row promised a "Cost is visible to Planners and Org Admins" banner; it was never built, and on review hiding is the better answer, so this row was corrected to the code rather than the code to this row. Verified: `cost:read` is granted to exactly Planner + Org Admin, the same set as `activity:update`. |
