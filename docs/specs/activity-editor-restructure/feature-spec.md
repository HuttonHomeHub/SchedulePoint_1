...
in nineteen with a size problem, and it has four specific, verifiable defects:

1. **The grouping is inverted for sighted users.** All three `<legend>`s are `sr-only`
   (`ActivityFormDialog.tsx:533, 653, 712`), with a duplicate `aria-hidden` paragraph carrying the
   visible text. A screen-reader user gets three named groups; a sighted user gets hairline
   `border-t` rules. Two sibling dialogs (`CalendarFormDialog`, `AddCrossPlanLinkDialog`) already
   use a **visible** legend, so this one is the outlier, not the convention.
2. **Ten of 22 fields are in no group at all** — an eight-field ungrouped preamble, and
   `Description` stranded after the last fieldset.
3. **The primary constraint is orphaned from the secondary.** `Constraint` / `Constraint date`
...
4. **The progress model is scattered across four surfaces** and one of them is
...
- **Zero ungrouped fields** and **zero `sr-only` legends** in the editor (structural test).
...
- No API, schema or engine change: `computeSchedule` is not imported, called or influenced. The
  ADR-0034 recalc parity gate is **structurally** untouched.
- Fields per visible screen drops from 22 to ≤ 10; the tallest tab (Scheduling) holds 10.

### Open questions

**CRITICAL — Q1. The Progress tab carries three Save buttons.** Co-locating schedule %, weighted
steps and physical % puts three _different write scopes_ on one tab: `PATCH …/progress`
(Contributor, no pen), `PATCH …/:activityId` (Planner + pen, for `% complete type` /
`physicalPercentComplete`), and `PUT …/steps` (Planner). They cannot share one button — see §4
"Why not one Save per tab, literally". **Default: three labelled panels, each with its own Save.**

**CRITICAL — Q2. How is the Steps panel gated?** Verified: the steps `PUT` is **not** pen-gated
server-side (`activity-steps.service.ts` calls `assertCan('activity:update', …)` and no
`assertHoldsPen`), and the two current hosts already disagree — `ActivitiesTable` gates the Steps
row action on `canWrite` (role only), `plan-dialogs.tsx` gates it on `model.canEditSchedule` (role
**and** pen). **Default: pen-gate it in the editor** (align with the canvas host), so the Progress
tab has exactly one boundary — "progress is yours, the activity's definition needs the lock" — and
record the API-side asymmetry in `TECH_DEBT.md` as defence-in-depth to fix separately.

**Q3 (softer). The editor stays open after a scope save.** Per-scope save implies it: closing would
strand the other tabs. Today `ActivityFormDialog` and `ActivityProgressDialog` close on save
(`ActivityStepsDialog` already does not), so this is a real change to muscle memory.
**Default: stay open, announce the save, Close is explicit.**
...
>   panel sits inside a fieldset with a **visible** legend.
...
| Progress → Weighted steps        | `PUT …/activities/:id/steps`               | `activity:update`                               | **No server-side** (Q2 — default: gate in the UI) | Planner, Org Admin              |
...
| Backend        | **none** | Verified: `UpdateActivityDto` documents "Every field is optional; send only what changes", and partial PATCHes are already in production use (`useSetActivityVisualStart`, `useRepositionLane` send two or three keys). No controller, service or DTO changes.                                                        |
| Database       | **none** | No model, column, index or constraint.                                                                                                                                                                                                                                                                                |
| API            | **none** | No new endpoint, no contract change, no OpenAPI delta. The editor composes three existing endpoints.                                                                                                                                                                                                                  |
| Security       | **low**  | No new permission, no new endpoint, no new data reaching the client. The change is which client surface _offers_ a write. Per-scope bodies **reduce** blast radius: a scope can no longer overwrite a field it does not show — the class of bug where a hidden-but-seeded field is re-sent on every save.             |
...
| Testing        | **high** | Unit + structural + flag-off parity + a11y + one flag-on journey. See §5 of the plan.                                                                                                                                                                                                                                 |
...
- **Nothing must land first.** Every endpoint, permission and flag it composes already exists.
...
|                | Weighted steps — _where physical % comes from_            | the step rows + live rollup                                                                                                                      | `PUT :id/steps` (Q2)              |
| **Cost**       | Cost                                                      | Budgeted expense, Actual expense, Cost accrual                                                                                                   | `PATCH :id` (pen)                 |

All 22 original fields are placed; nothing is dropped, nothing is ungrouped, every legend is
visible. `% complete type` moves out of "Cost & earned value" and onto **Progress**, beside the
three measures it selects between — the audit's fourth defect.
...
**Chosen:** a flagged, frontend-only tabbed editor composing the three existing endpoints, with one
Save per write scope, a new hand-rolled APG `Tabs` primitive, and the progress model co-located and
honestly shaded. Behind `VITE_ACTIVITY_EDITOR_TABS`, **default off**, with flag-off parity suites
pinning the three current dialogs as the rollback contract (the ADR-0053 M6 / ADR-0059 M6
precedent). Delivered as thin slices: primitive (dark) → pure schemas + gating (dark) → definition
tabs (flagged) → Progress co-location (flagged) → entry-point convergence → review gates + e2e →
flip.
...
- Docs this change updates: `docs/COMPONENT_LIBRARY.md` (the `Tabs` primitive),
  `docs/DESIGN_SYSTEM.md` (tab tokens/states if any are added), `docs/UX_STANDARDS.md` (dialog
  sectioning + the "one Save per write scope" rule), `CLAUDE.md` §16 (ADR-0060),
  `docs/TECH_DEBT.md` (the steps-PUT pen asymmetry; the eventual retirement of the superseded
  dialogs), `.github/workflows` (the new e2e step).
