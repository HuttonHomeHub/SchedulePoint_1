...
| --------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FC-1      | Held                             | `compute.finish-milestone.spec.ts`: placed on its predecessor's last day, no conflict and no drift; a day earlier still conflicts (control); also on a 24-hour calendar                                                                                                                                                                                                                                                                                             |
| FC-4      | Held                             | Pass 1 golden suite (`compute.spec.ts`) unedited and green                                                                                                                                                                                                                                                                                                                                                                                                          |
| FC-5      | **Wording corrected, then held** | As written ("no non-milestone row moves") it fails: 63 non-milestone rows move. Every one is downstream of a dated finish milestone (by edge, WBS parent or LOE span), which Q2 A accepts — the first wording forgot that a reinterpreted milestone pushes its successors. `scripts/measure-finish-milestone.mts`: 29 finish-milestone rows and 63 other rows changed, **0** outside the downstream set; the NetPoint control fired (GCO `2031-03-01 → 2031-02-28`) |
| FC-6      | Held                             | `apps/api/test/finish-milestone-date-migration.e2e-spec.ts`, through the real engine; verified red without the rewrite (the milestone reads the 12th and its successor the 13th)                                                                                                                                                                                                                                                                                    |
| FC-7      | Held                             | Engine unit case: an open-start finish milestone reads the data date, not the day before                                                                                                                                                                                                                                                                                                                                                                            |
| FC-8      | Held by construction             | Both of a finish milestone's axis days shift by one (`lib/milestone-day.ts`), so on a 24-hour calendar the diamond's pixel is unchanged; the paint golden log holds no finish milestone and passed unedited, so the unit case in `render-model.test.ts` carries it                                                                                                                                                                                                  |
| FC-9      | Held                             | Engine unit case: `FNLT` on the predecessor's last day gives float 0                                                                                                                                                                                                                                                                                                                                                                                                |

**Predictions.** NetPoint finish `2031-02-28`: confirmed in the harness. The torture prediction
(`2027-03-12 → 2027-03-11`, violation count stays 1) was stated about the **product** plan; the
harness builds engine input through `test/pairwise/spec-to-engine.ts`, which is known to schedule
that fixture differently (a four-month gap, `scripts/m0-engine-input.mts`), and reads
`2026-03-16 → 2026-03-13` with 0 violations before and after. The harness therefore neither confirms
nor refutes the product prediction; the torture plan's product dates are re-read from a seeded plan
before `docs/TEST_PLAYBOOK.md` is changed.
