...
| ~~`CHROME_RESIDUAL_PX = 56` over-charges Row 2 by ~47 px~~ — **WITHDRAWN, stale.** ADR-0091 M7 already fixed this: the constant is **`16`** today (`Toolbar.tsx:52`) and its docblock records recovering the 44 px. The figure describes the pre-M7 state. Verified by reading the file, 2026-08-18. | `Toolbar.tsx:40-52`                                                                   |
