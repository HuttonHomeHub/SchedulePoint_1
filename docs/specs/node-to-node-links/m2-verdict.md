# M2 verdict: node-to-node links. STOPPED at FC-T3 and put to the product owner
...
## Readings against the committed bars
...
## Not done, because the work stopped

M2's golden log, the `links.spec.ts` journey, the harness retirements, the FC-T8 cost reading, the
e2e sweep and the changeset. M4 (diagonals) is judged against M2's numbers, so it waits too.
