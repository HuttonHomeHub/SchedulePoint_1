...
**TWO cases invert the draw order — one per cap — and they are independent.** The rule is "draw the
window before the bars"; these are its **only** exceptions, and each draws after. Stated so they are
designed rather than discovered.

| Cap       | Inverts when                    | Mechanism                                                                                                                                                                                                                                         | Today                                                                                            |
| --------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| **Right** | remaining float is **negative** | US-5 — a placement past a ceiling, so the bar overflows its window to the right                                                                                                                                                                   | `floatTailRect` returns `null` for `totalFloatDays <= 0` (`geometry.ts:157`) ⇒ **draws nothing** |
| **Left**  | **drift** is negative           | US-1's **stay-and-flag** — `compute.ts:339` is `const display = placed ?? logicEarliest` with **no clamp**, so a placement earlier than logic allows is kept (and flagged at `:345`), and drift is **signed** (`packages/types/src/index.ts:645`) | `driftTailRect` returns `null` for `driftDays <= 0` (`geometry.ts:181`) ⇒ **draws nothing**      |

**The left cap was missing from this section until the ui-architect sign-off, and its absence is the
register's most-recorded failure shape** — one correct pattern applied to a control and not its
neighbour. The symmetry is exact down to today's behaviour: both states currently draw nothing, both
become visible under the window, and only one had been designed.

**Either can occur alone; on a long bar with negative float placed early, both occur at once.**

**The single-derivation decision is robust across sign, which is what makes this a draw-order fix
rather than a re-derivation.** With `d` negative the right cap is still
`placedFinish + remainingFloat = (earlyFinish + d) + (T − d) = earlyFinish + T = lateFinish`,
unchanged. Nothing about §4.8's "derive the right edge from `remainingFloat`, never independently
from `lateFinish`" needs qualifying for the negative case.
