...
- **Status:** **Delivered** — M1–M3 landed and released in `web-v0.62.0` (2026-07-31). Approved
  ...
  the current statement of behaviour.
...
- **TECH_DEBT #59.** ADR-0026 §9's ≤4 ms p95 @ 2,000 draw budget has never been
...
| Full orthogonal visibility graph + A* per edge | Correct and unbounded. Per-frame A* over thousands of edges cannot be reconciled with ADR-0026 §9, and path instability under pan/zoom would make links twitch. |
...
- **Consequences.** ADR-0026 §9's budget must be defended by a counting-stub gate of the
