...
**M1 (hour/shift-granular calendars + minute-based durations, ADR-0036) has landed**, clearing the
calendar/duration rows it gated. The remaining ❌ rows now sit behind **M2** (progress/actuals),
**M3** (per-relationship lag calendar), **M4** (mandatory/expected/secondary/ALAP constraints),
**M5** (LOE / resource-dependent / WBS / per-activity calendars) and **M6** (longest-path / multi-path
float) — not behind M1.
...
| Lag sign × type arithmetic                                 | `lag_zero/positive/negative`, `lag_{fs,ss,ff,sf}_*`, `lag_exceeds_pred_duration`                  | ✅     | M1 ✓             | Sign/type arithmetic in signed working-**minutes** (ADR-0036); the per-edge **24h lag calendar** override is seam-landed, wired in M3.                                              |
