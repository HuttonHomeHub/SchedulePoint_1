...
**M1 (hour/shift-granular calendars + minute-based durations, ADR-0036) has landed**, and **M3 wired the
24-Hour (elapsed) per-relationship lag calendar** (ADR-0036 §6 — the per-edge lag-calendar row is now 🟡:
24-Hour done, the Predecessor-vs-Successor half deferred to M5). The remaining ❌ rows sit behind **M2**
(progress/actuals), **M4** (mandatory/expected/secondary/ALAP constraints), **M5** (LOE /
resource-dependent / WBS / per-activity calendars) and **M6** (longest-path / multi-path float).
...
| Per-relationship lag calendar                              | `lag_calendar_24h`, `lag_calendar_setting_sensitive`                                              | 🟡     | M3 ✓ / M5        | **24-Hour (elapsed) lag wired (M3 ✓)** — the concrete-cure `A4430→A4440 FS+168h/24H` edge = 7 elapsed days; S06 is a runnable differential. `lag_calendar_setting_sensitive` (Predecessor-vs-Successor) needs per-**activity** calendars → M5; Pred/Succ coincide with the plan calendar until then. |
