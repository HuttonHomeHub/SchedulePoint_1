...
## P3 — TAKEN and PASSED (2026-09-09T08:20Z, product owner's hardware)

**The reading.** `revision-diff`, **Week** framing, 2,000 activities per side, 180 frames × 3, on
`web-v0.125.0` at 1912×948 CSS px, dpr 1, Intel Arc Pro Graphics, idle frame interval 16.70 ms.

|                      |                                                                                              |
| -------------------- | -------------------------------------------------------------------------------------------- |
| scene                | 2,160 bars (2,000 activities, 160 WBS summaries, 76 milestones), 3,200 links across 50 lanes |
| on screen            | 264 bars at 12.00 px/day                                                                     |
| drawn by the overlay | **37 / 264 bars (14.0 %)**, **49 / 372 links (13.2 %)**                                      |
| baseline             | **0.00 pp**, run-to-run spread **0.00 pp**                                                   |
| treatment            | **0.00 pp** at **60.0 fps**                                                                  |
| delta                | **+0.00 pp** against the 2.00 pp bar                                                         |
| **verdict**          | **PASS**                                                                                     |

**Both limbs of the rule are satisfied, and the second is the one that matters.** A delta inside the
bar decides nothing by itself; the spread limb is what separates an answer from a shrug, and at
0.00 pp the machine could resolve the question. Had the spread reached 2.00 pp this would have been
recorded INDETERMINATE and disqualified, exactly as ADR-0127 D8 was.

**Not vacuous.** The overlay drew 37 changed bars and 49 changed links while it was measured. A pair
with nothing to draw produces the same number for the wrong reason — which is why the drawn-ghost
count was made a task step rather than a closing formality.

**The prediction holds.** ADR-0127 D8a's same-plan reading was baseline 0.19 pp / treatment 0.00 pp
at 60.0 fps; cross-plan is 0.00 / 0.00 at 60.0 fps. The cross-plan rule narrows what is drawn and
costs no more, so **the lane clause did not leak back in**.

**Limits, restated rather than dropped.** One framing, one machine. **Fit stays ungraded**
(`docs/TECH_DEBT.md` #260 — a 98.33 pp baseline there leaves that delta arithmetically incapable of
failing, which is the opposite failure to this run's), and Week culls 2,160 bars to 264, so this is a
statement about the framing a planner works at rather than about every framing. A 0.00 pp baseline
also cannot resolve a cost below one dropped frame in 540 — far under the bar, so it does not touch
the verdict.

---

### What this section said while the reading was owed (recorded 2026-09-08, M2-T6)
