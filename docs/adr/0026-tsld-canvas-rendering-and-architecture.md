...
### 9b. Amendment (2026-08-03) — the device confirmation §9a deferred, and a numbering correction

§9a closed with one item open: _"**final device-fps confirmation** (mid-tier laptop + iPad-class
tablet, light/dark) is folded into M1 on real hardware — the one measurement the headless CI
environment cannot make."_ It was never made. `docs/TECH_DEBT.md` #59 recorded that for months, and
#75 inherited it. **It has now been made**, on the laptop half of the envelope.

**Method.** `apps/web/scripts/measure-draw-in-browser.js` — pasted into DevTools against the running
application, wrapping `requestAnimationFrame` to record per-frame callback cost and, more
importantly, the **interval between frames**. The plan is a 2,016-activity programme (48 WBS
summaries, 8 milestones, 3,793 dependencies, 5-day 8-hour calendar, ~7-month span) generated by
`packages/interchange/scripts/generate-scale-xer.mjs` and imported through the product's own
importer — not a synthetic scene handed straight to the painter.

**Hardware.** Dell Precision 5690, Intel Core Ultra 7 165H (22 threads), 64 GB, **mains**, Edge 151,
60 Hz, DPR 1, canvas 1036×646 CSS px, GPU `ANGLE (Intel, Intel(R) Arc(TM) Pro Graphics, D3D11)` —
the **integrated** adapter, which is the one the browser chose on a machine that also has a discrete
card, and therefore the one a planner actually gets.
...
The prototype's prediction held remarkably well. §9a measured **4.0 ms p95** at 2,000 activities on
a throwaway renderer drawing bars, links and a grid. The shipped canvas now also draws month bands,
a WBS band, float and drift tails, non-working hatching, flanking dates, arrowheads, lag runs,
handles and obstacle-aware link routing — every one an accepted decision with its own ADR — and
measures **3.9 ms** at the working zoom on a real imported programme.

**What the real-hardware run adds, and it is the part worth keeping.** At Fit, rAF JS is 8.9 ms —
half a frame — and 10% of frames still drop, at intervals of almost exactly 2× and 3× the refresh
period. §9a chose per-frame draw time as its metric **because** headless fps "is rAF-throttled with
no GPU compositor and only a floor", and said so; this is that proxy's limit appearing exactly where
§9a predicted the real-device measurement would be needed. The ~8 ms per frame not accounted for by
rAF JS is **unattributed** — raster, GPU upload, compositing and non-rAF main-thread work are all
invisible to the script, and zero long tasks > 50 ms rules out a blocking stall. It is not
