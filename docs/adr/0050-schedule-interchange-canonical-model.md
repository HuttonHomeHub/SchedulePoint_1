...
| `CALENDAR` / `<Calendar>`                                                                    | `Calendar` (+ `CalendarShift` + `CalendarException`) | weekday work-hours → shifts, holidays/exceptions → exception windows (ADR-0036); tier via `clndr_type` (ADR-0053 §5, MSPDI reports a drop). Windows are **repaired, not verbatim**: sorted, empty ones dropped, overlapping ones merged — each a reported repair (see below)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `CALENDAR.day_hr_cnt` / —                                                                    | `Calendar.hoursPerDay` (ADR-0068)                    | **mapped both ways** in XER (absent ⇒ the target derives it); MSPDI has no per-calendar equivalent, so export **reports a drop**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `CALENDAR.day_hr_cnt` below 0.25 h                                                           | `Calendar.hoursPerDay` = `0.25`                      | **repaired + reported**: stored as `Math.round(hours × 60)`, so a smaller value rounds to zero minutes and hits the column CHECK — a 500 during commit naming a constraint rather than a calendar                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| — / —                                                                                        | `ResourceAssignment.lagMinutes` (ADR-0071 §1)        | **not imported, dropped on export — both formats, both reported.** The canonical model carries the field; **no parser writes it and no emitter reads it.** This repository's own P6-class fixture declares `TASKRSRC` as `taskrsrc_id task_id rsrc_id target_qty target_qty_per_hr driving_flag act_reg_qty` — **no lag column** — so the P6 field's name, units and even existence are unknown here and will not be guessed (ADR-0058). MSPDI's `<Assignment><Delay>` is a plausible equivalent, read against no real file, and sits in the same confirm-first bucket. Import reports the capability **unconditionally** wherever assignments exist (it cannot know whether anything was lost); export reports **only when a non-zero lag is present** (it knows exactly), with a count. Unblocked by inspecting a real export carrying one — at which point this row records the column name and units |
| XER `UDFVALUE` labelled `SchedulePoint layout v1: placed start` / MSPDI —                    | `Activity.visualStart` (one-planning-surface)        | **XER: read and written exactly** (ADR-0156, which supersedes this row's earlier "not imported, absent from the canonical model"). The placement travels in a SchedulePoint-namespaced user-defined field that only SchedulePoint reads; the canonical model carries it as `layout.placedStart` only for a format that writes it. Import restores it by default (`restoreLayout=RESTORE`) and names it in the report when declined. Export reports ONE aggregate `approximation` (P6 and other tools show computed dates). **MSPDI: not imported, one aggregate `drop` on export** — unchanged until a real MS Project file has been read (#386, the deferred M5). A foreign file carries no placement, so nothing is reported for it                                                                                                                                                                    |
| XER `UDFVALUE` labelled `SchedulePoint layout v1: lane` (`TASK` and `PROJWBS`) / MSPDI —     | `Activity.laneIndex`                                 | **XER: read and written exactly** (ADR-0156). Restored lanes are left where the file put them (phase 3 `carried`/`partial`, never re-packed), and an overlap among them is reported as an `approximation` naming Arrange, not resolved. A file with no lanes is packed on the drawn span, keyed by activity code (ADR-0069 as amended). Lanes alone are never reported on export: no other tool has lanes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `TASK.task_type` / `<Task>` `Summary`+`Milestone` flags                                      | `ActivityType`                                       | XER exact both ways incl. `TT_LOE` ⇄ `LEVEL_OF_EFFORT`; MSPDI has no LOE (written as a task, **reported per activity**); P6 hammock → `TASK` + reported                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| UDFs, activity-code matrices, cost accounts, roles, expenses, risk, financial periods, steps | —                                                    | **dropped + reported** (out of scope, brief §3/§8 Won't-have). Foreign UDF rows produce **one** `drop` with a count — true only since ADR-0156; before it nothing reported them. SchedulePoint's own two layout labels are the exception above                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
