...
| — / —                                                                                        | `Activity.visualStart` (one-planning-surface)        | **not imported — and nothing is reported, which is the opposite of the row above and deliberate.** A hand-placement is a SchedulePoint concept: P6 and MSPDI carry constraints and computed dates, never "a human put this bar here". So unlike `lagMinutes` there is no candidate column awaiting a real file, the importer knows **with certainty** that nothing was lost, and a standing import finding would be noise on every import forever. For the same reason the field is **absent from the canonical model** — a slot there would reserve space for something that can never arrive — and reaches the export graph only. **Export reports ONE aggregate drop when any activity carries one**, with a count, from a single producer in `export-mapper.ts`; a structural test pins that neither serialiser mentions the field                                                                   |
