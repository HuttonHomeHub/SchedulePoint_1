...
| `CALENDAR` / `<Calendar>`                                                                    | `Calendar` (+ `CalendarShift` + `CalendarException`) | weekday work-hours → shifts **verbatim**, holidays/exceptions → exception windows (ADR-0036); tier via `clndr_type` (ADR-0053 §5, MSPDI reports a drop) |
