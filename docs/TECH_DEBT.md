...
**Status:** open · **Verified:** 2026-09-10 (same-day — see the caveat on #286) · **Raised:** 2026-09-10 (workspace-console M7, the architecture review) · **Size:** S ·
**Owner:** web
...
### 290. Two Gantt date cells open, accept a keystroke, and refuse every value

**Status:** open · **Verified:** 2026-09-10 · **Raised:** 2026-09-10 (gantt-editing-gaps spec) ·
**Size:** S · **Owner:** web

**Live on the auto-pulling host since `web-v0.92.0`** (ADR-0047 — every release reaches the product
owner's stack), so this is a shipped defect rather than an internal inconsistency.

Double-click a Gantt row's **Start** or **Finish** cell on a plan with a computed schedule and it
opens an editor. Type a date in exactly the format the cell displays, press Enter, and the grid
answers **"That value is not something this cell accepts."** — shown _and_ spoken. There is no
value it accepts. The refusal is unconditional:

- `cell-edit.ts:246-251` maps `earlyStart` and `earlyFinish` in `GANTT_EDITABLE_COLUMNS`;
- `cell-gate.ts:62-100` returns `writable: true` for them once a schedule exists and the row is
  neither a summary nor a milestone, so the cell lights and opens;
- `cell-commit.ts:87-93` returns `null` for both keys before parsing anything;
- `cell-commit.ts:137-143` turns that `null` into the message above.

**The reasoning behind the refusal is right, and that is what makes it survivable.** The engine owns
`earlyStart`; a client that PATCHed it would assert an answer rather than an input, so a typed date
must write the CONSTRAINT a drag writes. Refusing beats sending something plausible. What is wrong
is that the cell was left **lit** while the write path was refused — the ADR-0081 shape inverted:
not a capability with no entry point, but an entry point with no capability.

**Three things make it worse than a missing feature.**

1. **It blames the planner.** The message is about the value they typed. Every value is refused, so
   a planner reads it as "I typed the date wrong" and tries again.
2. **The task it defers to does not exist.** Both the code comment and the test cite `M2-T3b`;
   `grep -rn 'M2-T3b' docs/specs/` returns only the new spec discussing this. It defers to nothing.
3. **A unit test pins the dead end as correct.** `cell-commit.test.ts:60-66` asserts both keys
   return `null` under the heading "never PATCHes a computed date column" — true of the write and
   silent about the cell being open. ADR-0104's shape: a suite whose fixture is the broken state.

It also merged against the parent epic's own written merge condition — _"No milestone may merge with
an affordance that is visible and inert"_ (`docs/specs/gantt-editing/feature-spec.md:379-381`).

**The remedy is to withdraw the two cells, not to build the write.** Removing both keys from
`GANTT_EDITABLE_COLUMNS` makes the columns read-only, which is honest and is what the Gantt shipped
as; building the typed-date cell is a schedule semantic that needs an ADR, and is
`docs/specs/gantt-editing-gaps/` M3 awaiting approval. Withdrawal is a contained fix to behaviour
this row describes and adds no surface, so ADR-0105 does not fire for it — but it needs the pinning
test inverted in the same commit, or the suite will assert the defect back.

**Found by writing the `gantt-editing-gaps` spec, not by a report** — nothing in the product is
capable of noticing that a gate says `writable` and a commit says `null`, and the register carries
the finding separately from that spec so it survives the spec not being approved.
