...
| 82 | **Shift-editor epic (ADR-0067/0068) — the non-blocking half of five specialist gates** | The five M4 gates found ten blocking defects, all folded with regression tests. These are the rest, deferred by decision rather than missed: **(a)** the interchange importer writes calendar shift/exception windows verbatim without the ordering/overlap check the API's own DTO enforces, so a hostile or broken file reaches the engine's `validateWindows` and surfaces as an opaque 500 rather than a dry-run finding (security, suggested); the same path lets a near-zero `day_hr_cnt` round to 0 minutes and hit the DB CHECK. **(b)** No `@ArrayMaxSize` on the new `shifts`/`windows` DTO arrays — bounded in practice by the 100 kB JSON body limit, but out of step with the codebase's own `@ArrayMaxSize` convention. **(c)** `addException`/`removeException` do not take the `calendar:manage_org` check `updateException` now takes for an ORG-scoped calendar; behaviourally inert today (every role with `calendar:update` has it) but a latent inconsistency. **(d)** The calendar library table does not summarise a shift calendar, so a split-shift and a plain Mon–Fri calendar look identical in the list (`maxWindowsPerDay` exists and has no consumer). **(e)** Window validation fires on Save, not on blur. **(f)** An overlapping pair flags only the second row. **(g)** No unit test files for `exception-hours.ts`, `shift-summary.ts` and `window-rows.ts` — each is covered only through its consumers. | Low each. (a) is the only one with a user-visible failure mode, and it is self-inflicted by the importing organisation's own uploader with a clean rollback. | (a) validate window ordering in the pure mapper and report it as a repair finding, which is where every other import approximation already goes; the rest as ordinary follow-ups. |
