...
| 8   | **The web app serves no Content-Security-Policy at all**             | Corrected 2026-08-04 — the row said "not finalised", which reads as a policy that exists and could be tighter. `apps/web/nginx.conf` sets `X-Content-Type-Options`, `X-Frame-Options` and `Referrer-Policy` and **no `Content-Security-Policy` header**, so the SPA has none. The API's `helmet()` default does set one, but that governs JSON responses and is not the document policy a browser applies to the app. The stated reason — "origins/assets unknown until features land" — expired: the features landed, and the origins are now knowable by reading the code (self-hosted assets, no CDN, no external fonts, `blob:`/`data:` for the canvas image and PDF export paths).                                   | The app renders user-authored text — activity names, notes, plan titles, imported XER/MSPDI fields — so an injected-script bug would have no second line of defence. This is the highest-value unclaimed security control in the repo.                                                                                                                          | Write the policy from what the code actually loads, ship it `Content-Security-Policy-Report-Only` first, then enforce. Mind the canvas export (`blob:`), the lazily-imported jsPDF chunk, and the `unsafe-inline` style question Tailwind v4 raises. Run it past **security-reviewer** before enforcing. |
...
### 94. A verification email that never sends is invisible to everyone

**Found:** 2026-08-04, by the reconciliation pass's step-7 security review over PR #225 — the commit
that introduced the application's first outbound network transport and was never specialist-reviewed.

`SmtpMailService.sendEmailVerification` throws on a send failure, deliberately, and three places said
that failure fails the sign-up: its own docblock, `better-auth.ts`'s `sendVerificationEmail` contract,
and `docs/DEPLOYMENT.md`. **It does not.** Better Auth calls the port through
`ctx.context.runInBackgroundOrAwait(...)` (`better-auth@1.6.25`, `api/routes/sign-up.mjs:246`), whose
...
`[better-auth] Failed to run background task` line — outside Pino, outside the correlation-id
pipeline, outside the redaction rules, and therefore outside whatever the deployment aggregates. The
user sees a normal sign-up and an account they cannot use; the operator sees nothing.

**Risk:** low frequency, high confusion. It needs a misconfigured or failing SMTP relay to bite, and
