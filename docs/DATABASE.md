...
> models across 58 committed migrations — is the single source of truth for the data model.
> See ADR-0008.
...
| `idx_clients_org_deleted_at`                  | `(organization_id, deleted_at DESC, id)` | partial        | the recycle-bin list's filter **and** its mixed `(deleted_at DESC, id ASC)` sort (`WHERE deleted_at IS NOT NULL`), and the organisation-agnostic hierarchy-expiry scan `deleted_at < :cutoff` — which the partial index serves because a strict operator implies `IS NOT NULL`. Sized by the deleted set, so a live-row INSERT does no index maintenance. Measured (see the migration): one screen open 1,208 ms → 466 ms at 8,773 deleted rows; the idle hourly expiry run 13–19 ms → 0.7–1.4 ms |
| `idx_projects_org_deleted_at`                 | `(organization_id, deleted_at DESC, id)` | partial        | the projects branch of the same two reads                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `idx_plans_org_deleted_at`                    | `(organization_id, deleted_at DESC, id)` | partial        | the plans branch of the same two reads — the largest of the three. `id` is in the key because a cascade stamps **one** `deleted_at` on every row it touches, so paging a big batch is entirely the id tiebreak                                                                                                                                                                                                                                                                                    |
...
| `idx_activities_parent_id_fk`                 | `(parent_id)`                            | partial        | the **`parent_id` FK RESTRICT check**, `WHERE parent_id IS NOT NULL`. `idx_activities_parent_id` cannot serve it: a RESTRICT check's generated query carries no `deleted_at`, so a `deleted_at IS NULL` predicate is not implied and the check scans the whole table once per hard-deleted row (measured 3m47s for a 2,000-activity plan). Partial because only an activity under a WBS summary has a parent — 32 kB against 1,384 kB full                                                        |
...
| `idx_resource_assignments_activity_id_fk`     | `(activity_id)`                          | full           | the **`activity_id` FK RESTRICT check**; both `uq_resource_assignments_*` indexes are `deleted_at`-predicated and cannot serve it. Full because `activity_id` is `NOT NULL`, so a predicate would be a tautology                                                                                                                                                                                                                                                                                  |
...
| `idx_activity_steps_activity_id_fk`           | `(activity_id)`                          | full           | the **`activity_id` FK RESTRICT check**; `uq_activity_steps_activity_seq` is `deleted_at`-predicated. Declared via `@@index(map:)` — Prisma can see a non-partial index, so omitting it from the schema is drift                                                                                                                                                                                                                                                                                  |
...
| `idx_notes_activity_id_fk`                    | `(activity_id)`                          | partial        | the **`activity_id` FK RESTRICT check**, `WHERE activity_id IS NOT NULL` (a PLAN note has none); `idx_notes_activity_created` is `deleted_at`-predicated                                                                                                                                                                                                                                                                                                                                          |
