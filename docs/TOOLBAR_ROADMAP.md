...
| `float-paths`            | find   | look | inline       | **Float paths** — ✅ **Wired (`VITE_FLOAT_PATHS`, default on; audit F4).** Toggles the docked **Float paths** panel: the ranked contiguous driving chains into the selected activity (ADR-0035 §19), each with its relative float above the driving path, and one selected path emphasised in **both** the canvas and the Gantt. A pure read — no pen, every role. Needs a selection (it suggests the current one); shades with a reason when the plan has no activities.                                      |
...
| `export`                 | object | do   | inline       | **Export** — ✅ **Wired (`VITE_EXPORT_PRINT`).** Grouped Export ▾ menu: Schedule / All activities (CSV, injection-safe + BOM; conditional "Matching activities only (N)" under a lens) + Diagram whole-plan/current-view as PNG (off-screen `paintScene`) and PDF (lazy jsPDF). XER/MSP **import** shipped separately on the plan-create surface (Stage C2, `VITE_SCHEDULE_INTERCHANGE`, ADR-0050); XER/MSP **export** deferred to interchange M4. View-only.                                                  |
...
| `share`                  | object | do   | inline       | **Share** — ✅ **Wired (`VITE_GUEST_SHARE_LINKS`, default off during build; ADR-0051 F-M4).** Opens the member **Share links** dialog (list / create — with the one-time guest URL + Copy — / revoke a plan's External-Guest links), gated on `plan:share` (Planner + Org Admin; shaded-with-reason otherwise). Not pen-gated (sharing grants read access, it doesn't author). The public read-only `/share` guest view is the paired surface. Flag-off it stays the "Coming soon" placeholder, byte-for-byte. |
