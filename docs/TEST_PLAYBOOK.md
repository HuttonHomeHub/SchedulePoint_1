...
| 10 Resources                 | `plan:scale-500`                                    | **310 of 478** unassigned (64.9 %) — the generator assigns 35 % of tasks, so a 100 % figure means the assignments never landed. `INFORMATIONAL`, `threshold: null`, narrowing named. The denominator may read **482** if the engine writes a duration back to the four `LEVEL_OF_EFFORT` rows; not yet observed (`docs/TECH_DEBT.md` #224) | **`478 of 478` — what this row said until 2026-08-31.** A reader checking metric 10 against it would have accepted a 100 %-unassigned verdict from a fixture that is 35 % assigned. Also wrong: a pass/fail verdict, or a threshold object on an informational row |
