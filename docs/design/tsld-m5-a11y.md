...
| Reposition — time (`Alt+→`/drag)       | "Moved '{name}'; dates will update." (both paths append it on any time change; `+ " to lane N"` if lane too)                                                                       | stale version → move-conflict banner (shipped)                 |
| Reposition — lane (`Alt+↑↓`/drag)      | "Moved '{name}' to lane N." (shipped)                                                                                                                                              | stale version → move-conflict banner (shipped)                 |
| **Lane boundary (#24b)** — **NEW**     | `Alt+↑` at lane 0: "Already in the top lane." (Only lane 0 is bounded; downward is unbounded so a planner can spread activities into new lanes — a bottom cap would be arbitrary.) | —                                                              |
