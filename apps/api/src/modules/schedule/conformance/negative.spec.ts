...
describe('negative-case contract (engine-owned cases)', () => {
  it('N01: a three-activity FS cycle is rejected, naming the cycle members', () => {
    ...
    const edges: EngineEdge[] = [
      { id: 'e1', predecessorId: 'A', successorId: 'B', type: 'FS', lagDays: 0 },
      { id: 'e2', predecessorId: 'B', successorId: 'C', type: 'FS', lagDays: 0 },
      { id: 'e3', predecessorId: 'C', successorId: 'A', type: 'FS', lagDays: 0 },
    ];
    const error = caught(() =>
      computeSchedule(activities, edges, { dataDate, calendar: allDaysWorkCalendar }),
    );
  ...
  it('N02: a self-loop is rejected, naming the activity', () => {
    const error = caught(() =>
      computeSchedule(
        [task('A')],
        [{ id: 'e1', predecessorId: 'A', successorId: 'A', type: 'FS', lagDays: 0 }],
  ...
  it('N03: a cycle that exists only through SS/FF edges is still caught (not FS-only detection)', () => {
    const edges: EngineEdge[] = [
      { id: 'e1', predecessorId: 'A', successorId: 'B', type: 'SS', lagDays: 0 },
      { id: 'e2', predecessorId: 'B', successorId: 'A', type: 'FF', lagDays: 0 },
    ];
    const error = caught(() =>
      computeSchedule([task('A'), task('B')], edges, { dataDate, calendar: allDaysWorkCalendar }),
    );
    expect(error).toBeInstanceOf(ScheduleGraphNotADagError);
    expect([...(error as ScheduleGraphNotADagError).unresolvedActivityIds].sort()).toEqual([
      'A',
  ...
  it('N05: a relationship to a non-existent activity is rejected, naming it', () => {
    const error = caught(() =>
      computeSchedule(
        [task('A')],
        [{ id: 'e1', predecessorId: 'A', successorId: 'GHOST', type: 'FS', lagDays: 0 }],
        { dataDate, calendar: allDaysWorkCalendar },
      ),
    );
  ...
  it('N11: a calendar with no working time is rejected at construction (the hang-test analogue)', () => {
    // The day-granular analogue of the zero-working-hour calendar: an empty weekday
    // mask would make addWorkingDays non-terminating, so the factory refuses it —
    // report, never hang. ADR-0036 restores this as a "no working time in horizon" check.
  ...
  it('N16: an enormous lag terminates quickly with a finite date (no walker hang)', () => {
    ...
    const output = computeSchedule(
      [task('A'), task('B')],
      [{ id: 'e1', predecessorId: 'A', successorId: 'B', type: 'FS', lagDays: 12_500 }],
      { dataDate: '2026-01-05', calendar: monFri },
    );
    const b = output.results.find((r) => r.activityId === 'B');
    expect(b?.earlyStart).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    // ~12,500 working days ≈ 48 calendar years out — comfortably finite and far future.
    expect(Number(b!.earlyStart.slice(0, 4))).toBeGreaterThan(2060);
  });
