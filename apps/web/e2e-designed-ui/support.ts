...
export async function onboard(page: Page, stamp: number): Promise<string> {
  const orgSlug = `designed-co-${stamp}`;
  await page.goto('/sign-up');
  await page.getByLabel('Full name').fill('Designed Tester');
  await page.getByLabel('Email').fill(`designed-${stamp}@example.com`);
  await page.getByLabel('Password').fill('correct-horse-battery');
  await page.getByRole('button', { name: /create account/i }).click();
  await expect(page.getByRole('heading', { name: /create your organisation/i })).toBeVisible();
  await page.getByLabel('Organisation name').fill(`Designed Co ${stamp}`);
