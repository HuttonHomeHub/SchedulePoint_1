...
export async function seedActivities(
  page: Page,
  orgSlug: string,
  specs: readonly { name: string; durationDays?: number; startOn?: string }[],
  ...
  const result = await page.evaluate(
    async ({
      ...
      org: string;
      id: string;
      rows: readonly { name: string; durationDays?: number; startOn?: string }[];
    }) => {
      ...
      for (const [index, row] of rows.entries()) {
        const response = await fetch(`/api/v1/organizations/${org}/plans/${id}/activities`, {
          ...
          body: JSON.stringify({
            ...
            laneIndex: index,
            ...(row.startOn === undefined
              ? {}
              : { constraintType: 'SNET', constraintDate: row.startOn }),
          }),
        });
        if (!response.ok) {
