 ...
 *    the Gantt with the dates the engine actually computed, without a second fetch or a second
 *    date-to-pixel implementation. This is what "a peer view of one model" has to mean.
 * 2. **The view choice is in the URL.** `?view=gantt` survives a reload and is shareable — the
 *    ADR-0053 M6 precedent that view state belongs in typed search params, proven by a real reload
 *    rather than by a router mock.
 * 3. **The grid is readable without sight of it.** The bars are decoration; the dates live in
 *    cells. A real axe pass over the rendered treegrid is the only way to know that survived.
 *
...
test('the Gantt is a peer view of the same schedule, deep-linkable and readable as a grid', async ({
  ...
  await drawActivity(page, 'Excavate', { x: 160, y: 120 });
  await drawActivity(page, 'Piling', { x: 360, y: 200 });
  ...
  // Both authored activities are rows, and their dates are TEXT in cells — not encoded only in a
  // bar. The plan starts 2026-01-05, so the first activity's computed start is a January date.
  const excavate = grid.getByRole('row', { name: /Excavate/ });
  await expect(excavate).toBeVisible();
  await expect(grid.getByRole('row', { name: /Piling/ })).toBeVisible();
  ...
  await expect(grid.getByRole('row', { name: /Piling/ })).toBeFocused();
