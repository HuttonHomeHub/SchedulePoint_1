 ...
 *
 * Four forms in the recovery flow swap their whole `<form>` for an outcome — sign-in's unverified
 * branch, the reset request's "check your email", the reset's "password changed", the resend's
 * "on its way". In each, the just-clicked submit button is **unmounted**, so focus falls to
 * `<body>`: the visible focus ring vanishes and a keyboard user is returned to the top of the
 * document with no indication of where the outcome went. That is the WCAG 2.4.3 failure this
 * codebase has already treated as blocking twice (ADR-0064's split-button caret, ADR-0060 M6's
 * Save button).
 *
