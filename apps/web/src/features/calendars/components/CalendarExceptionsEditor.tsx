...
/**
 * Manage a calendar's dated exceptions: list the existing overrides (each with a
 * working/holiday text indicator, not colour alone) with a Remove action, plus a
 * small add form. A duplicate-date conflict (409) is surfaced as a friendly
 * inline message. Fetches the calendar detail itself so it can stay embedded in
 * the edit dialog.
 */
export function CalendarExceptionsEditor({
  ...
  const onRemove = (exception: CalendarExceptionSummary): void => {
    removeException.mutate(exception.id, {
      onSuccess: () => announce(`Exception on ${formatCalendarDate(exception.date)} removed.`),
    });
  };
