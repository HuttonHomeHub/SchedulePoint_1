...
describe('SelectionActionsBar (floating selection actions)', () => {
  it('renders nothing when nothing is selected', () => {
    const { container } = render(<SelectionActionsBar anchorRef={{ current: null }} ctx={null} />);
    expect(container).toBeEmptyDOMElement();
    expect(screen.queryByRole('toolbar')).not.toBeInTheDocument();
  });

  it('shows the object actions for the selected activity, named after it', () => {
    render(<SelectionActionsBar anchorRef={anchorRef} ctx={ctx()} />);
  ...
  it('runs the read action (open logic) even in read-only', () => {
    render(<SelectionActionsBar anchorRef={anchorRef} ctx={ctx({ canEditSchedule: false })} />);
  ...
  it('runs the mutating actions when editing is allowed', () => {
    render(<SelectionActionsBar anchorRef={anchorRef} ctx={ctx({ canEditSchedule: true })} />);
