...
/**
 * The pure, renderer-agnostic TSLD render model (ADR-0026) — **the barrel**.
 *
 * Every consumer in the repository imports from here and none of them changed when the geometry
 * core moved out (ADR-0078 §3's barrel-preserving rule, `docs/TECH_DEBT.md` #106). What is left in
 * this file is link routing and the model built on the core; the core itself is `./geometry`, which
 * imports nothing from here — that acyclicity is the point, not a side effect.
 */
