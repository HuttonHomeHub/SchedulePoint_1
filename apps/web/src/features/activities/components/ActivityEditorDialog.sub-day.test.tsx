import type { ActivitySummary, CalendarSummary } from '@repo/types';
import { WorkingWeekdays } from '@repo/types';
import { QueryClient, QueryClientProvider } from '@tanstack/react-query';
import { fireEvent, render, screen, waitFor, within } from '@testing-library/react';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';

import { deriveActivityEditorGating } from '../lib/activity-editor-gating';

import { ActivityEditorDialog } from './ActivityEditorDialog';

/**
 * The duration field with `VITE_SUB_DAY_DURATIONS` on (ADR-0070), ported from the deleted edit path
 * of `ActivityCreateDialog.sub-day.test.tsx` (activity-dialog-unification epic). Both flags default
 * on already; pinned explicitly for the same reason the deleted suite did — this file is ABOUT them.
 *
 * The claims that matter here are the ones a pure parser test cannot make: that the field reads the
 * factor from the calendar the SCHEDULING scope currently selects (not the saved one — a planner can
 * change both in one visit, ADR-0070 §3), that a General save sends `durationMinutes` rather than
 * `durationDays`, and that the field degrades honestly to whole working days when the calendar list
 * has not resolved. `apps/web/e2e-sub-day/` depends on this same behaviour end to end.
 */
vi.mock('@/config/env', async (importOriginal) => ({
  ...(await importOriginal<Record<string, unknown>>()),
  SUB_DAY_DURATIONS_ENABLED: true,
  ACTIVITY_CALENDAR_ENABLED: true,
}));

const PATCHES: { url: string; body: Record<string, unknown> }[] = [];

function calendar(id: string, name: string, hoursPerDay: number): CalendarSummary {
  return {
    id,
    name,
    description: null,
    workingWeekdays: 0b0011111,
    shifts: WorkingWeekdays.toFullDayShifts(0b0011111),
    hoursPerDay,
    hoursPerDayMinutes: Math.round(hoursPerDay * 60),
    scope: 'ORG',
    projectId: null,
    archivedAt: null,
    version: 1,
    createdAt: '2026-01-01T00:00:00Z',
    updatedAt: '2026-01-01T00:00:00Z',
  };
}

/** An eight-hour plan calendar and a 24-hour one — the same "1d" is a different number of minutes. */
const CALENDARS = [
  calendar('cal-8', 'Eight-hour week', 8),
  calendar('cal-24', 'Round the clock', 24),
];

/** Four hours on the eight-hour calendar — the shape the whole epic exists for. */
function activity(overrides: Partial<ActivitySummary> = {}): ActivitySummary {
  return {
    id: 'a1',
    planId: 'pl1',
    name: 'Lift steel',
    code: 'A100',
    type: 'TASK',
    durationType: 'FIXED_DURATION_AND_UNITS_TIME',
    durationDays: 0,
    durationMinutes: 240,
    percentCompleteType: 'DURATION',
    accrualType: 'UNIFORM',
    calendarId: null,
    parentId: null,
    version: 4,
    ...overrides,
  } as ActivitySummary;
}

const PLANNER_WITH_PEN = deriveActivityEditorGating({
  penManaged: true,
  holdsPen: true,
  canWrite: true,
  canProgress: true,
  canReadCost: true,
});

function mount(props: Partial<Parameters<typeof ActivityEditorDialog>[0]> = {}) {
  const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
  return render(
    <QueryClientProvider client={client}>
      <ActivityEditorDialog
        orgSlug="acme"
        planId="pl1"
        open
        onClose={() => {}}
        activity={activity()}
        gating={PLANNER_WITH_PEN}
        calendars={CALENDARS}
        planCalendarId="cal-8"
        {...props}
      />
    </QueryClientProvider>,
  );
}

const openTab = (name: string): void => {
  fireEvent.click(screen.getByRole('tab', { name }));
};

const comboField = (): HTMLElement => screen.getByRole('combobox', { name: 'Calendar' });
/** Open the popup exactly as a keyboard user does (APG: ↓ opens at the first option). */
const openCombo = (): void => {
  fireEvent.keyDown(comboField(), { key: 'ArrowDown' });
};
const chooseCombo = (option: string | RegExp): void => {
  openCombo();
  fireEvent.pointerDown(within(screen.getByRole('listbox')).getByRole('option', { name: option }));
};

beforeEach(() => {
  PATCHES.length = 0;
  vi.stubGlobal(
    'fetch',
    vi.fn((url: string, init?: RequestInit) => {
      const method = (init?.method ?? 'GET').toUpperCase();
      if (method !== 'GET') {
        const body: string = typeof init?.body === 'string' ? init.body : '{}';
        PATCHES.push({ url, body: JSON.parse(body) as Record<string, unknown> });
      }
      return Promise.resolve({
        ok: true,
        status: 200,
        json: () => Promise.resolve({ data: activity({ version: 5 }) }),
      } as unknown as Response);
    }),
  );
});

afterEach(() => vi.unstubAllGlobals());

describe('ActivityEditorDialog — sub-day durations', () => {
  it('seeds a sub-day duration as text instead of showing it as 0 days', () => {
    mount();
    expect(screen.getByLabelText('Duration')).toHaveValue('4h');
  });

  it('names the calendar’s own day length, because 1d is not a fixed quantity', () => {
    mount();
    expect(screen.getByText(/A day is 8 working hours/)).toBeInTheDocument();
  });

  it('sends exact minutes, never durationDays', async () => {
    mount();
    fireEvent.change(screen.getByLabelText('Duration'), { target: { value: '2d 4h' } });
    fireEvent.click(screen.getByRole('button', { name: /save general/i }));

    await waitFor(() => expect(PATCHES).toHaveLength(1));
    expect(PATCHES[0]!.body.durationMinutes).toBe(1200); // 2 × 480 + 240
    expect(PATCHES[0]!.body).not.toHaveProperty('durationDays');
  });

  it('follows the calendar the FORM selects, not the one the row was saved on', async () => {
    mount();
    // Move the activity onto the 24-hour calendar, on the Scheduling scope — then come back to
    // General and enter a duration. "1d" must now mean 1440, before that scope has even saved.
    openTab('Scheduling');
    chooseCombo('Round the clock');
    openTab('General');
    fireEvent.change(screen.getByLabelText('Duration'), { target: { value: '1d' } });
    fireEvent.click(screen.getByRole('button', { name: /save general/i }));

    await waitFor(() => expect(PATCHES).toHaveLength(1));
    expect(PATCHES[0]!.body.durationMinutes).toBe(1440);
  });

  it('refuses a unit it does not have, naming the ones it does', async () => {
    mount();
    fireEvent.change(screen.getByLabelText('Duration'), { target: { value: '1w' } });
    fireEvent.click(screen.getByRole('button', { name: /save general/i }));

    // ONCE, beside the field — `FormProblemCount` is silent at one problem because `handleSubmit`
    // has already moved focus to the offending control (ADR-0077 §9).
    expect(await screen.findAllByText(/Weeks are not supported/)).toHaveLength(1);
    expect(screen.queryByText(/problems — check the highlighted fields below/)).toBeNull();
    expect(PATCHES).toHaveLength(0);
  });

  it('degrades to whole working days — and says so — when the factor is unknown', () => {
    // No calendar list, so nothing resolves the day length. Days is the one unit that needs no
    // factor; guessing 24 would read a planner's "1d" as three days of eight-hour work.
    mount({ calendars: [] });
    expect(screen.getByLabelText('Duration (working days)')).toHaveValue(0);
    expect(screen.queryByText(/A day is/)).not.toBeInTheDocument();
  });
});
