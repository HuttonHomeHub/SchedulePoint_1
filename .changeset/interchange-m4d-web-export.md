...
- **Export menu items** — the canvas **Export ▾** menu gains an "Interchange" group with **Primavera P6
  (.xer)** and **Microsoft Project (.xml)**, after the existing CSV/PNG/PDF/Print items (no second menu).
  The whole group renders only when the `VITE_SCHEDULE_INTERCHANGE` flag AND the caller's
  `interchange:export` permission are both true — the latter is held by every member (Viewer upward), so
  most users see it. Flag-off / permission-off ⇒ the menu is byte-for-byte the Stage-C1 set.
...
- **Report surfacing** — after a successful download, the interchange report is surfaced unobtrusively via
  the shared polite announcer; when the export approximated/dropped anything (notably MSPDI), the report
  text is auto-downloaded alongside the file so the lossiness is never hidden (the header stays the source
  of truth).
