...
Notes M3 — the flagged web surface (the Notes feature, ADR-0046) — behind a **new default-off flag
`VITE_NOTES`**, so it changes nothing until an operator opts in. It puts the live note-thread API
(M2) in front of a planner: attributed, time-ordered note threads on plans and activities — the
weekly-progress "why", not just the "what".
...
flag-on Playwright journey (`playwright.notes.config.ts`, wired into CI). Flag OFF ⇒ existing behaviour
byte-identical (no sections mount, the counts query never fires, the badge is suppressed).
