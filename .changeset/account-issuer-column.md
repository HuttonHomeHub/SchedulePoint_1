...
land unattended inside the API's self-migrating entrypoint. It is four steps instead: add nullable,
backfill guarded on `provider_id`, `SET NOT NULL` (which aborts loudly on anything the guard missed),
then the default and Better Auth's declared `UNIQUE (issuer, account_id)`.
