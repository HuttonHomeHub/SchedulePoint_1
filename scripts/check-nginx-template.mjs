...
const problems = [];
const rendered = substitute(read(TEMPLATE), composeDefaults());
...
for (const [, name, quote, value] of rendered.matchAll(
  ...
  if (value.includes(quote)) {
    problems.push(
      `${TEMPLATE} — add_header ${name}: the value is wrapped in ${quote} and contains ${quote}.\n` +
...
// A value carrying BOTH quote characters cannot be expressed by swapping delimiters, so the advice
// above would be wrong. Say so rather than sending someone round a loop.
for (const [, name, , value] of rendered.matchAll(
  /add_header\s+(\S+)\s+(["'])(.*?)\2\s*(?:always)?\s*;/g,
)) {
  if (value.includes('"') && value.includes("'")) {
    problems.push(
      `${TEMPLATE} — add_header ${name}: the value contains both quote characters, so no choice of\n` +
        `    delimiter works. It needs escaping or restructuring, not a swap.`,
    );
  }
}
...
console.log(`nginx template OK (${TEMPLATE} substitutes cleanly with ${COMPOSE}'s CSP defaults).`);
