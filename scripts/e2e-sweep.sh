...
#
# **Not a per-change step.** Thirty-three suites is about forty minutes. It belongs to a change that
...
set -u
SUITES="${*:-staff edit toolbar toolbar-fit workspace-chrome authoring authoring-flow programme notes undo loe multi-select copy-paste resource-view interchange share library calendar-shifts sub-day assignment-lag float-paths search-nav designed-ui designed-chrome gantt activity-editor wbs recently-deleted audit account account-verify public overview gantt-editing}"
for s in $SUITES; do
